...
void CClientManager::QUERY_LOGIN_BY_KEY(CPeer* pkPeer, DWORD dwHandle, TPacketGDLoginByKey* p)
{
	...
	strlcpy(pkTab->country, r.country, sizeof(pkTab->country));
#endif
	...
	snprintf(szQuery, sizeof(szQuery), "SELECT `pid1`"
		", `pid2`"
		", `pid3`"
		", `pid4`"
...
#endif
		", `empire`"
		" FROM player_index%s WHERE `id` = %u", GetTablePostfix(), r.id);
...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer* peer, SQLMsg* msg)
{
	...
	snprintf(szQuery, sizeof(szQuery),
		"SELECT `id`, `name`, `job`, `level`, `playtime`, `st`, `ht`, `dx`, `iq`, "
#if defined(__CONQUEROR_LEVEL__)
		"`conqueror_level`, `sungma_str`, `sungma_hp`, `sungma_move`, `sungma_immune`, "
#endif
		"`part_main`, `part_hair`, "
		...
		"UNIX_TIMESTAMP(`last_play`) FROM player%s WHERE `account_id` = %u",
...
#endif
		", `empire`"
		" FROM player_index%s WHERE `id` = %u", GetTablePostfix(),
...
#if defined(__CONQUEROR_LEVEL__)
					pkTab->players[j].byConquerorLevel = pt->conqueror_level;
					pkTab->players[j].bySungmaStr = pt->sungma_str;
					pkTab->players[j].bySungmaHp = pt->sungma_hp;
					pkTab->players[j].bySungmaMove = pt->sungma_move;
					pkTab->players[j].bySungmaImmune = pt->sungma_immune;
#endif
					pkTab->players[j].dwMainPart = pt->adwParts[PART_MAIN];
					pkTab->players[j].dwHairPart = pt->adwParts[PART_HAIR];
#if defined(__ACCE_COSTUME_SYSTEM__)
					pkTab->players[j].dwAccePart = pt->adwParts[PART_ACCE];
#endif
					pkTab->players[j].x = pt->x;
					pkTab->players[j].y = pt->y;
					pkTab->players[j].skill_group = pt->skill_group;
					pkTab->players[j].bChangeName = 0;
					pkTab->players[j].last_play = GetLastPlayTime(player_id); //pt->last_play;
				}
				else
				{
					if (!row[col++])
					...
					pkTab->players[j].bySungmaStr = 0;
					pkTab->players[j].bySungmaHp = 0;
					pkTab->players[j].bySungmaMove = 0;
					pkTab->players[j].bySungmaImmune = 0;
#endif
					pkTab->players[j].dwMainPart = 0;
					pkTab->players[j].dwHairPart = 0;
#if defined(__ACCE_COSTUME_SYSTEM__)
					...
					str_to_number(pkTab->players[j].bySungmaStr, row[col++]);
					str_to_number(pkTab->players[j].bySungmaHp, row[col++]);
					str_to_number(pkTab->players[j].bySungmaMove, row[col++]);
					str_to_number(pkTab->players[j].bySungmaImmune, row[col++]);
#endif
					str_to_number(pkTab->players[j].dwMainPart, row[col++]);
					str_to_number(pkTab->players[j].dwHairPart, row[col++]);
#if defined(__ACCE_COSTUME_SYSTEM__)
					...
					str_to_number(pkTab->players[j].last_play, row[col++]);
				}

				sys_log(0, "%s %lu %lu hair %u",
					pkTab->players[j].szName, pkTab->players[j].x, pkTab->players[j].y, pkTab->players[j].dwHairPart);
				break;
			}
		}
		/*
